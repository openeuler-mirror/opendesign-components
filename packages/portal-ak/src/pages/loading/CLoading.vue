...
import { ref } from 'vue';
import { OLoading } from '@components/index';
...
<template>
  <OLoading :class="[`c-loading-${props.size}`]" v-bind="$attrs">
    <template #icon>
      <div class="c-loading-icon o-rotating"></div>
      <!-- <div class="c-loading-numbers">
        <svg viewBox="0 0 21 32" class="c-loading-number">
          <path
            fill="#4f4f4f"
            style="fill: var(--color1, #4f4f4f)"
...
.c-loading-number {
  // width: 21px;
  // height: 32px;
  width: calc(var(--loading-icon-size) / 8);
}
</style>
