...
- 可选值：`cover` | `contain` | `auto` | `&lt;length&gt;` | `&lt;percentage&gt;` - 示例：`fit="100% 80%"` 可设置自定义背景图尺寸 - 注意：由于不再使用 `<img>` 标签渲染，可能导致宽高塌陷
      ...
      **`preview`**: 点击预览功能，启用后会接管点击事件（显示全屏预览层）。传 `true` 使用默认配置，传对象时作为 `OImageViewer`
      的属性配置（如 `zoomRate`、`minScale`、`toolbar` 等）
      ...
      - 启用后组件渲染为 `<a>` 标签
        ...
        **`fit`**: Image fill mode (Default: `contain`)
        ...
        - Optional values: `cover` | `contain` | `auto` | `&lt;length&gt;` | `&lt;percentage&gt;`
            ...
            **`preview`**: Click-to-preview functionality. When enabled, it takes over the click event (displays a
            full-screen preview layer). Pass `true` for default config, or an object to forward `OImageViewer` props (e.g.
            `zoomRate`, `minScale`, `toolbar`).
            ...
            **`href`**: Link navigation
            ...
            - When enabled, the component renders as an `<a>` tag
              ...
              The `preview` and `href` properties **cannot be enabled simultaneously** (both would take over the click
              event).
</docs>
