...
3. 通过 `detailMaxRow` 设置内容最大行数，高于该行数时默认显示渐隐，可以通过设置 `showFadeOut` 为 `false` 来显示省略号。**注**：通过`-webkit-line-clamp`实现；一般而言 `detailRow` 的值与 `detailMaxRow` 值应该一致
...
3. Set the maximum number of lines for the content through `detailMaxRow`. When it exceeds the number of lines, fade-out will be displayed by default. You can display the ellipsis by setting `showFadeOut` to `false`.
