...
通过 `shortcut` slot 提供快捷日期入口，slot 提供 `setValue(value)` 和 `emitChange()` 方法。

<!-- en-US -->

### Shortcuts

Use the `shortcut` slot to add preset date options. The slot exposes `setValue(value)` and `emitChange()`.
</docs>
<script setup lang="ts">
import { ref } from 'vue';
import dayjs from 'dayjs';
import { ODatePicker, OLink, OForm, OFormItem, type DatePickerShortcutSlotProps } from '@opensig/opendesign';
