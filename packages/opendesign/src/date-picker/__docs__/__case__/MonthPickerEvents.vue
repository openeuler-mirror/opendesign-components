...
`OMonthPicker` supports the following events:
...
import { OMonthPicker, OForm, OFormItem } from '@opensig/opendesign';
...
const onFocus = (evt: FocusEvent) => {
  console.log('[month-picker] focus', evt);
};
const onBlur = () => {
  console.log('[month-picker] blur');
};
const onChange = (newVal: number | undefined, oldVal: number | undefined) => {
  console.log('[month-picker] change', { newVal, oldVal });
};
const onClear = (evt?: Event) => {
  console.log('[month-picker] clear', evt);
};
const onPressEnter = () => {
  console.log('[month-picker] pressEnter');
};
</script>
<template>
  <OForm :layout="lePadV ? 'v' : 'h'" label-width="80px">
    <OFormItem label="月份">
      <OMonthPicker v-model="val" clearable @focus="onFocus" @blur="onBlur" @change="onChange" @clear="onClear" @press-enter="onPressEnter" />
      <template #extra>
