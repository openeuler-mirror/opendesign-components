...
`ODatePicker` supports the following events:
...
const onFocus = (evt: FocusEvent) => {
  console.log('[date-picker] focus', evt);
};
const onBlur = () => {
  console.log('[date-picker] blur');
};
const onChange = (newVal: number | undefined, oldVal: number | undefined) => {
  console.log('[date-picker] change', { newVal, oldVal });
};
const onClear = (evt?: Event) => {
  console.log('[date-picker] clear', evt);
};
const onPressEnter = () => {
  console.log('[date-picker] pressEnter');
};
</script>
<template>
  <OForm :layout="lePadV ? 'v' : 'h'" label-width="80px">
    <OFormItem label="日期选择">
      <ODatePicker v-model="val" clearable @focus="onFocus" @blur="onBlur" @change="onChange" @clear="onClear" @press-enter="onPressEnter" />
      <template #extra>
