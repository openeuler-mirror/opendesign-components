...
`OYearPicker` supports the following events:
...
import { OYearPicker, OForm, OFormItem } from '@opensig/opendesign';
...
const onFocus = (evt: FocusEvent) => {
  console.log('[year-picker] focus', evt);
};
const onBlur = () => {
  console.log('[year-picker] blur');
};
const onChange = (newVal: number | undefined, oldVal: number | undefined) => {
  console.log('[year-picker] change', { newVal, oldVal });
};
const onClear = (evt?: Event) => {
  console.log('[year-picker] clear', evt);
};
const onPressEnter = () => {
  console.log('[year-picker] pressEnter');
};
</script>
<template>
  <OForm :layout="lePadV ? 'v' : 'h'" label-width="80px">
    <OFormItem label="年份">
      <OYearPicker v-model="val" clearable @focus="onFocus" @blur="onBlur" @change="onChange" @clear="onClear" @press-enter="onPressEnter" />
      <template #extra>
