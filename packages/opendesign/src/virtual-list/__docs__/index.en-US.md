...
| Per-item height ^[NEXT](primary) | `(item, index) => number` | Items have different but computable heights | Exact                                  |
...
- `scrollToView(index, align?, behavior?)`: Scrolls the item at the given index into view. `align` supports `'start'` / `'end'` / `'center'` / `'nearest'` / `number` (pixel offset). In dynamic-height mode, if the target item is not yet measured, it first aligns to `start` to trigger rendering, then re-positions via a "two-pass approach" ^[NEXT](primary) after measurement.
- `scrollToOffset(px)` ^[NEXT](primary): Scrolls directly to the specified pixel offset, automatically clamped to the valid range.
...
| \-\-\_vl-content-width ^[NEXT](primary) | auto    | Total content width in horizontal mode                                                                          |
| \-\-\_vl-offset-y ^[NEXT](primary)      | 0px     | Y-axis transform offset of the render list in vertical mode                                                     |
| \-\-\_vl-offset-x ^[NEXT](primary)      | 0px     | X-axis transform offset of the render list in horizontal mode                                                   |
