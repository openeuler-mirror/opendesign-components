...
| 按项定高 ^[NEXT](primary) | `(item, index) => number` | 每项高度不一致但可由函数计算 | 精确                 |
...
- `scrollToView(index, align?, behavior?)`：将指定索引项滚动到视口内。`align` 支持 `'start'` / `'end'` / `'center'` / `'nearest'` / `number`（偏移像素）。不定高模式下若目标项尚未测量，会先以 `start` 对齐触发渲染，测量后通过"二次逼近"^[NEXT](primary)重新定位。
- `scrollToOffset(px)` ^[NEXT](primary)：直接滚动到指定像素偏移量，自动 clamp 到合法范围。
...
| \-\-\_vl-content-width ^[NEXT](primary) | auto   | 水平模式下的内容总宽度                                                           |
| \-\-\_vl-offset-y ^[NEXT](primary)      | 0px    | 垂直模式下渲染列表的 Y 轴 transform 偏移                                         |
| \-\-\_vl-offset-x ^[NEXT](primary)      | 0px    | 水平模式下渲染列表的 X 轴 transform 偏移                                         |
